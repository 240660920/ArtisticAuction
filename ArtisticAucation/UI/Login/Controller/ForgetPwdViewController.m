...
@interface ForgetPwdViewController ()<UITextFieldDelegate,UITableViewDataSource,UITableViewDelegate>
...
-(void)resetPassword
{
    ...
    [SMSSDK commitVerificationCode:self.verifyCodeTf.text phoneNumber:self.phoneNumTf.text zone:@"86" result:^(NSError *error) {
        if (!error) {
            ...
            [HttpManager requestWithAPI:@"user/resetPwd" params:@{@"phone" : self.phoneNumTf.text , @"password" : [self.passwordTf.text MD5Digest]} requestMethod:@"POST" completion:^(ASIFormDataRequest *request) {
                ...
                if (respose && respose.result.resultCode.intValue == 0) {
                    [self.view showHudAndAutoDismiss:@"密码重置成功"];
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        [self.navigationController popViewControllerAnimated:YES];
                    });
                }
                else{
                    [self.view showHudAndAutoDismiss:@"操作失败"];
                }
...
-(RegistTextField *)confirmPasswordTf
{
    if (!_confirmPasswordTf) {
        ...
        _confirmPasswordTf.placeholder = @"确认新密码";
...
-(UIButton *)registButton
{
    if (!_registButton) {
        ...
        [_registButton setTitle:@"重置密码" forState:UIControlStateNormal];
        ...
        [_registButton addTarget:self action:@selector(resetPassword) forControlEvents:UIControlEventTouchUpInside];
