...
/**zhuangzq,2015.09.25,监听转屏事件，修改位置**********************************/
...
-(void)show{
/**zhuangzq,2015.09.25,显示前先修改布局**********************************/
    [self setNeedsLayout];
/***zhuangzq,2015.09.25修改结束****************************************************/
    [[UIApplication sharedApplication].keyWindow addSubview:self];
    
}
/**zhuangzq,2015.09.25,增加界面布局代码**********************************/
- (void)layoutSubviews
{
    [super layoutSubviews];
    //ios8以下，UIWindow不转屏，需要修改界面布局
    if ([[[UIDevice currentDevice] systemVersion] floatValue] < 8.0) {
        ...
        if (UIInterfaceOrientationIsLandscape(orientation)) {
            ...
            if (self.toolbar) {
                ...
                frame.size.width = [self getScreenSize].width;
            ...
            //横屏home键在右边，先旋转90度，后平移
            if (orientation == UIInterfaceOrientationLandscapeRight) {
                CGFloat xTranslation = -(self.center.y - [UIScreen mainScreen].bounds.size.height / 2);
                CGFloat yTranslation = -([UIScreen mainScreen].bounds.size.width / 2 - self.bounds.size.height / 2);
                CGAffineTransform transform = CGAffineTransformMakeRotation(M_PI_2);
                transform = CGAffineTransformConcat(transform, CGAffineTransformMakeTranslation(yTranslation, xTranslation));
                self.transform = transform;
            }
            //横屏home键在左边，先旋转-90度，后平移
            else
            {
                CGFloat xTranslation = -(self.center.y - [UIScreen mainScreen].bounds.size.height / 2);
                CGFloat yTranslation = ([UIScreen mainScreen].bounds.size.width / 2 - self.bounds.size.height / 2);
