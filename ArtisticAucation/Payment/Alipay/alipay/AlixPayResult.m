...
- (id)initWithString:(NSString *)string{
    if (self = [super init]) {
        ...
        NSDictionary * jsonQuery = [NSJSONSerialization JSONObjectWithData:[string dataUsingEncoding:NSUTF8StringEncoding] options:0 error:NULL];
...
- (id)initWithDic:(NSDictionary *)jsonQuery {
    ...
    if (self = [super init]) {
        ...
        //        {
        do {
            self.statusCode=[[jsonQuery objectForKey:@"resultStatus"] integerValue];
            if (self.statusCode != 9000) {
                break;
            }
